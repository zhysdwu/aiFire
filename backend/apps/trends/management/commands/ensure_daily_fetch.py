...
from apps.trends.models import DailyFetchCheckpoint


class Command(BaseCommand):
    help = "Ensure only one successful fetch per day based on checkpoint date."
    ...
    def handle(self, *args, **options):
        ...
        try:
            if not self._checkpoint_table_exists():
                self.stdout.write(
                    self.style.WARNING("未检测到每日检查点表，请先执行数据库迁移：python manage.py migrate")
                )
                return
            checkpoint = DailyFetchCheckpoint.get_default()
        except (ProgrammingError, OperationalError):
            self.stdout.write(
                self.style.WARNING("数据库结构尚未更新，请先执行数据库迁移：python manage.py migrate")
            )
            return
        ...
        if checkpoint.last_success_date == today:
            self.stdout.write(
                self.style.SUCCESS(
                    f"今日已抓取成功，跳过。日期：{checkpoint.last_success_date}，时间：{checkpoint.last_success_at}"
                )
            )
            return
